...
class KernajangController extends Controller
{
    public function index()
    {
        ...
        $carts=DB::table('tbl_keranjang')->select('tbl_keranjang.*',\DB::raw("'produk' as types"),\DB::raw('tbl_produk.img1 as image'), \DB::raw('tbl_produk.nama_brand as name'))->join('tbl_produk', 'tbl_produk.kode_produk', '=', 'tbl_keranjang.id_produk')->where('session',session('id_cart'))->whereIn('type',['bus','pesawat','hotel'])
        ->union($rent)
        ->union($paket)
        ->get();
